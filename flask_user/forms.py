...
def unique_username_validator(form, field):
    ...
    if not user_manager.db_manager.username_is_available(field.data):
        raise ValidationError(_('Kullanımda olan bir kullanıcı adı girdiniz. Lütfen başka bir tane deneyiniz'))
...
class ChangePasswordForm(FlaskForm):
    ...
    old_password = PasswordField(_('Eski Parola'), validators=[
        validators.DataRequired(_('Eski parola zorunlu')),
        ])
    new_password = PasswordField(_('Yeni Parola'), validators=[
        validators.DataRequired(_('Yeni parola zorunlu')),
        password_validator,
        ])
    retype_password = PasswordField(_('Yeni parolayı tekrarlayın'), validators=[
        validators.EqualTo('new_password', message=_('Yeni parolalar uyuşmuyor'))
        ])
    submit = SubmitField(_('Parolayı değiştir'))
    ...
    def validate(self):
        ...
        if not current_user or not user_manager.verify_password(self.old_password.data, current_user.password):
            self.old_password.errors.append(_('Eski parola yanlış'))
            return False
...
class ChangeUsernameForm(FlaskForm):
    ...
    new_username = StringField(_('Yeni kullanıcı adı'), validators=[
        validators.DataRequired(_('Kullanıcı adı zorunlu')),
        username_validator,
    ...
    submit = SubmitField(_('Kullanıcı adını değiştir'))
...
class EditUserProfileForm(FlaskForm):
    ...
    first_name = StringField(_('Adınız'), validators=[validators.DataRequired()])
    last_name = StringField(_('Soyadınız'), validators=[validators.DataRequired()])
...
class LoginForm(FlaskForm):
    ...
    username = StringField(_('Kullanıcı Adı'), validators=[
        validators.DataRequired(_('Kullanıcı adı zorunlu')),
    ])
    email = StringField(_('Email'), validators=[
        validators.DataRequired(_('Email is required')),
    ...
    password = PasswordField(_('Parola'), validators=[
        validators.DataRequired(_('Parola zorunlu')),
    ])
    remember_me = BooleanField(_('Beni hatırla'))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user_manager.USER_ENABLE_USERNAME and user_manager.USER_ENABLE_EMAIL:
            ...
            self.username.label.text = _('Kullanıcı adı veya Eposta')
    ...
    def validate(self):
        ...
        if user_manager.USER_ENABLE_USERNAME and user_manager.USER_ENABLE_EMAIL:
            ...
            username_or_email_text = (_('Kullanıcı adı/Eposta'))
        ...
        if show_does_not_exist:
            if not user:
                message = _('%(username_or_email) bulunamadı', username_or_email=username_or_email_text)
                username_or_email_field.errors.append(message)
            else:
                self.password.errors.append(_('Parolanızı mı unuttunuz?'))

        # Always show 'incorrect username/email or password' error message for additional security
        else:
            message = _('Geçersiz %(username_or_email)s ve/veya parola', username_or_email=username_or_email_text)
            username_or_email_field.errors.append(message)
...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField(_('Parola'), validators=[
        validators.DataRequired(_('Parola zorunlu')),
        password_validator])
    retype_password = PasswordField(_('Parolayı yeniden girin'), validators=[
        validators.EqualTo('password', message=_('Parolalar uyuşmuyor'))])
